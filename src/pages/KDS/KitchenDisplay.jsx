  ...
  const getStatusInfo = (status) => {
    switch (status) {
      case "new":
        return {
          label: "NEW",
          color: "bg-emerald-50 text-emerald-600 border-2 border-emerald-100",
        };
      case "progress":
        return {
          label: "PREPPING",
          color: "bg-blue-50 text-blue-600 border-2 border-blue-100",
        };
      case "ready":
        return {
          label: "DISPATCHED",
      ...
      case "completed":
        return {
          label: "COMPLETED",
          color: "bg-gray-100 text-gray-900 border-2 border-gray-200",
        };
      default:
        return {
          label: "ORDER",
          color: "bg-gray-50 text-gray-400 border-2 border-gray-100",
        };
    }
  };
    ...
    <div
      ...
      className={`group relative bg-white rounded-[40px] border-4 border-gray-200 p-8 cursor-pointer transition-all hover:border-gray-900 flex flex-col justify-between shadow-xl transform active:scale-[0.98] ${
        order.status === "progress"
          ? "border-t-[12px] border-t-blue-600"
          : order.status === "ready"
            ? "border-t-[12px] border-t-orange-600"
            : order.type?.toLowerCase() === "dine in"
              ? "border-l-[12px] border-l-emerald-500"
              : "border-l-[12px] border-l-orange-500"
      }`}
    >
      <div className="flex justify-between items-start">
        <div className="flex flex-col">
          ...
          {order.table && (
            <span className="text-[10px] font-black text-gray-400 uppercase mt-2 tracking-widest">
              Table {order.table}
            </span>
          )}
        </div>
        <span
          className={`text-[10px] font-black px-4 py-1.5 rounded-full uppercase tracking-widest shadow-sm ${statusInfo.color}`}
        >
          {statusInfo.label}
        </span>
      </div>
        ...
        {items.length > 0 ? (
          items.slice(0, 3).map((item, i) => (
            <div key={i} className="flex gap-4 items-center">
              <span className="text-sm font-black text-gray-900 bg-gray-100 w-8 h-8 rounded-xl flex items-center justify-center border-2 border-gray-200">
                {item.qty}x
              </span>
              <span className="text-lg font-bold text-gray-700 leading-tight truncate uppercase tracking-tight">
                {item.name}
              </span>
            </div>
          ))
        ) : (
          <p className="text-xs font-bold text-red-500/50 uppercase italic tracking-widest">
            No items found
          </p>
        )}
      </div>
...
export default function KitchenDisplay() {
  ...
  useEffect(() => {
    ...
    const q = query(
      ordersRef,
      where("status", "in", ["new", "progress", "ready"]),
    );
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      allCompleted.sort(
        (a, b) =>
          (b.completedAtRaw?.seconds || 0) - (a.completedAtRaw?.seconds || 0),
      );
  ...
  const renderMainContent = () => {
    ...
    switch (activeNav) {
      ...
      case "PREP":
        ...
        break;
      case "HISTORY":
        const filteredCompleted = completedList.filter((o) =>
          o.id.includes(searchQuery),
        );
        return (
          <div className="flex-1 p-8 grid grid-cols-2 lg:grid-cols-4 auto-rows-max content-start gap-8 overflow-y-auto custom-scrollbar bg-gray-100 min-h-0">
          ...
          <div className="flex-1 p-10 bg-white border-4 border-gray-200 rounded-[48px] m-8 overflow-y-auto custom-scrollbar shadow-2xl min-h-0">
            <div className="max-w-5xl mx-auto space-y-4">
              ...
              {[...orders, ...completedList]
                .filter((o) => o.id.includes(searchQuery))
                .slice(0, 50)
                .map((o) => (
                  <div
                    key={o.id}
                  ...
                  >
                    ...
                    <span className="text-xs text-gray-700 font-bold truncate pr-6">
                      {o.items?.[0]?.name || "..."}
                    </span>
      ...
      <div className="flex-1 p-8 grid grid-cols-2 lg:grid-cols-4 auto-rows-max content-start gap-8 overflow-y-auto custom-scrollbar bg-gray-100 min-h-0">
        ...
        {displayOrders.length === 0 && (
          <div className="col-span-full h-full flex flex-col items-center justify-center opacity-30 gap-6">
            ...
            <span className="text-xs font-black tracking-[0.5em] text-gray-400 uppercase italic">
              {emptyLabel}
            </span>
          </div>
        )}
  ...
  if (isLoading) {
    return (
      <div className="h-screen w-screen bg-gray-50 flex items-center justify-center">
        <div className="text-gray-900 font-black animate-pulse text-2xl tracking-[0.4em] uppercase italic">
          System Ready
        </div>
      </div>
    );
    ...
    <div className="h-screen w-screen bg-gray-100 text-gray-900 flex flex-col font-sans overflow-hidden select-none">
      ...
      <div className="flex items-center justify-between px-10 py-6 border-b-4 border-gray-200 bg-white shrink-0 shadow-sm z-10">
        <div className="text-[12px] font-black tracking-[0.4em] text-gray-900 flex items-center gap-4">
          <div className="w-10 h-10 bg-gray-900 rounded-2xl flex items-center justify-center text-lg text-white shadow-xl italic font-black">
            V
          </div>
        ...
        <div className="w-[240px] bg-white border-l-4 border-gray-200 flex flex-col shrink-0 overflow-hidden shadow-sm">
          ...
          <div className="flex-1 space-y-3 overflow-y-auto custom-scrollbar p-6">
            {completedList.slice(0, 15).map((item) => (
              ...
              >
                <span className="font-mono text-sm font-black text-gray-500 group-hover:text-gray-900 italic">
        ...
        >
          ...
          >
            <div className="p-10 border-b-4 border-gray-100 flex justify-between items-center bg-white">
              <div>
                <p className="text-[10px] font-black tracking-[0.4em] text-gray-400 mb-2 uppercase">
                  Order Intelligence
                </p>
                <h2 className="text-5xl font-black text-gray-900 tracking-tighter italic uppercase">
                  Kitchen <span className="text-gray-400">Archive</span>
                </h2>
              </div>
              <button
                onClick={() => setShowAllHistory(false)}
                ...
                {completedList.map((item, idx) => (
                  ...
                  >
                    ...
                    <div className="text-center">
                      <p className="text-4xl font-black text-gray-900 mb-1 tracking-tighter italic">
                        #{item.id.slice(-4).toUpperCase()}
                      </p>
      ...
      <div className="flex items-center gap-12 px-10 py-4 border-t-4 border-gray-200 bg-white shrink-0 shadow-inner">
        ...
        <div className="flex gap-16">
          ...
          <StatItem
            label="PREPPING"
            value={orders.filter((o) => o.status === "progress").length}
            color="text-blue-600"
          />
      ...
      {selectedOrder && (
        ...
        >
          <div
            onClick={(e) => e.stopPropagation()}
            className="bg-white w-full max-w-2xl h-full max-h-[90vh] rounded-[64px] border-4 border-gray-200 shadow-[0_40px_120px_rgba(0,0,0,0.2)] overflow-hidden flex flex-col animate-in zoom-in-95 duration-200 cursor-default"
          >
            {/* Modal Header */}
            <div
              className={`p-10 flex justify-between items-start shrink-0 ${
                selectedOrder.type?.toLowerCase() === "dine in"
                  ? "bg-emerald-50"
                  : "bg-orange-50"
              }`}
            >
              <div>
                ...
                <h2 className="text-6xl font-black text-gray-900 tracking-tighter italic">
            ...
            {/* Modal Content */}
            <div className="flex-1 min-h-0 p-12 space-y-10 overflow-y-auto custom-scrollbar bg-white">
              <div className="flex items-center gap-8">
                <span
                  className={`px-6 py-2.5 rounded-2xl text-[11px] font-black tracking-[0.2em] border-2 ${
                    selectedOrder.type?.toLowerCase() === "dine in"
              ...
              <div className="space-y-6">
                {selectedOrder.items && selectedOrder.items.length > 0 ? (
                  selectedOrder.items.map((item, i) => (
                    <div
                      key={i}
                      className="flex items-start gap-8 bg-gray-50 p-8 rounded-[40px] border-2 border-gray-100 shadow-sm group transition-all hover:border-gray-900"
                    >
                      ...
                      <div className="flex-1 pt-2">
                        <p className="text-3xl font-black text-gray-900 leading-tight uppercase tracking-tight italic">
                          {item.name}
                        </p>
                  ...
                  <div className="p-20 text-center border-4 border-dashed border-gray-100 rounded-[64px] opacity-30">
                    <Utensils
                      size={80}
                      className="mx-auto mb-8 text-gray-200"
                    />
                    <p className="text-2xl font-black uppercase tracking-[0.3em] text-gray-300">
                      Ticket Empty
                    </p>
                  </div>
                )}
            ...
            {/* Modal Actions */}
            <div className="p-12 bg-white border-t-4 border-gray-100 grid grid-cols-1 gap-6 shadow-[0_-20px_50px_rgba(0,0,0,0.02)] shrink-0">
              {selectedOrder.status === "new" ? (
                <button
                  onClick={() =>
                    handleUpdateStatus(selectedOrder.id, "progress")
                  }
                ...
                <div className="flex flex-col gap-8">
                  <div className="bg-blue-50 border-2 border-blue-100 p-8 rounded-[40px] text-center shadow-inner">
                    <p className="text-[11px] font-black text-blue-600 uppercase tracking-[0.5em] mb-3">
                      Unit Deployment
                    </p>
                    <p className="text-gray-900 font-black text-2xl tracking-tighter italic">
                      {selectedOrder.assignedRobot
                        ? `Robot ${selectedOrder.assignedRobot.toUpperCase()} is En Route`
                        : "Waiting for Available Unit..."}
                    </p>
                  </div>
                  <button
                    onClick={() =>
...
function StatItem({ label, value, color }) {
  return (
    <div className="flex flex-col items-center">
      <span className="text-[8px] font-black tracking-[0.2em] text-[#444] mb-0.5">
        {label}
      </span>
      <span
        className={`font-mono text-lg font-black leading-none tabular-nums ${color}`}
      >
        {value}
      </span>
    </div>
