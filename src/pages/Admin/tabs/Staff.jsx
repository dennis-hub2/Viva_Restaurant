...
const StaffTab = () => {
  ...
  const handleAddStaff = async (e) => {
    ...
    try {
      ...
      setNewStaff({ name: "", role: "Server", status: "Active", shift: "Morning", avatar: "👨‍🍳" });
    } catch (error) {
  ...
  return (
    <div className="w-full text-left">
      <header className="mb-10 flex justify-between items-end">
        ...
        <div className="text-xs bg-[#6539A3] text-white px-3 py-1 rounded-full font-black uppercase tracking-widest">
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {staff.map((person) => (
            <div key={person.id} className="bg-[#2A2A2D] p-6 rounded-[32px] border border-white/5 flex flex-col gap-4 shadow-2xl group hover:border-white/10 transition-colors relative overflow-hidden">
              <button 
                  ...
                  <div className="flex gap-2">
                    <button 
                      ...
                      className={`px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-widest border transition-colors ${
                        person.status === 'Active' ? 'bg-green-500/10 text-green-400 border-green-500/20 hover:bg-green-500/20' : 
                        person.status === 'On Break' ? 'bg-yellow-500/10 text-yellow-400 border-yellow-500/20 hover:bg-yellow-500/20' : 
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#1A1A1D] w-full max-w-md rounded-[40px] border border-white/10 p-8 shadow-2xl animate-in zoom-in-95 duration-200">
            <h3 className="text-2xl font-black text-white mb-6">New Staff Member</h3>
            <form onSubmit={handleAddStaff} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest block mb-1">Role</label>
                  <select 
                    value={newStaff.role}
                ...
                <div>
                  <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest block mb-1">Shift</label>
                  <select 
              ...
              <div>
                <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest block mb-1">Avatar Emoji</label>
                <input 
                  type="text" 
